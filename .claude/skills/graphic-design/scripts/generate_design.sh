...
# Usage: ./generate_design.sh "design purpose" [style keywords] [color preferences]
...
if [ -z "$DESIGN_PURPOSE" ]; then
    echo "Usage: $0 \"design purpose\" [style keywords] [color preferences]"
    echo "Example: $0 \"LinkedIn banner\" \"modern minimal\" \"blue tones\""
    exit 1
fi
...
if [ -n "$STYLE_KEYWORDS" ] && [ -n "$COLOR_PREFERENCES" ]; then
    DETAILED_PROMPT="A modern, minimal graphic design for $DESIGN_PURPOSE, clean layout, $COLOR_PREFERENCES color palette, subtle gradients, strong visual hierarchy, professional aesthetic, spacious composition, high resolution, soft lighting, no clutter, $STYLE_KEYWORDS style"
elif [ -n "$STYLE_KEYWORDS" ]; then
    DETAILED_PROMPT="A modern, minimal graphic design for $DESIGN_PURPOSE, clean layout, muted neutral colors, subtle gradients, strong visual hierarchy, professional aesthetic, spacious composition, high resolution, soft lighting, no clutter, $STYLE_KEYWORDS style"
elif [ -n "$COLOR_PREFERENCES" ]; then
    DETAILED_PROMPT="A modern, minimal graphic design for $DESIGN_PURPOSE, clean layout, $COLOR_PREFERENCES color palette, subtle gradients, strong visual hierarchy, professional aesthetic, spacious composition, high resolution, soft lighting, no clutter"
else
    DETAILED_PROMPT="A modern, minimal graphic design for $DESIGN_PURPOSE, clean layout, soft neutral background, subtle gradients, strong visual hierarchy, professional aesthetic, spacious composition, high resolution, soft lighting, no clutter"
fi
...
echo "4. Generating 1-3 high-quality images"
...
SAMPLE_FILE_1="../../../graphic-designs/${CLEAN_PURPOSE}-01_${TIMESTAMP}.png"
SAMPLE_FILE_2="../../../graphic-designs/${CLEAN_PURPOSE}-02_${TIMESTAMP}.png"
...
echo "- Generate actual images based on the prompt"
...
echo "- Files saved: graphic-designs/${CLEAN_PURPOSE}-*.png"
